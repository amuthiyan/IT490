...
function requestProcessor($request)
{
  ...
  LogMsg("received request",$PathArray[4], "amuthiyan", "DevDMZ");
  echo "received request".PHP_EOL;
  var_dump($request);
  if(!isset($request['type']))
  {
    LogMsg("ERROR: unsupported message type",$PathArray[4], "amuthiyan", "DevDMZ");
    return "ERROR: unsupported message type";
  }
  switch ($request['type'])
  {
    case "all_cards":
      LogMsg('Sending all card tags now',$PathArray[4], "amuthiyan", "DevDMZ");
      return getCardTags();
    case "get_card":
      LogMsg('Sending requested card info now',$PathArray[4], "amuthiyan", "DevDMZ");
...
LogMsg("Trying to start primary server test",$PathArray[4], "amuthiyan","DevDMZ");
